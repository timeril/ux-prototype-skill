...
name: ux-prototype
description: >
  Apply this skill whenever the user wants to design, critique, plan, or prototype any digital product — screens, flows, wireframes, navigation structures, UI components, or user journeys. Trigger even if the user doesn't use UX-specific language: "help me design an app", "sketch out how this feature would work", "what screens would this need", "map out the flow", "prototype this idea", "how should users accomplish X", or "what would this look like" are all triggers. This skill runs a Lean UX-grounded process that surfaces user context before touching screens, generates opinionated design artifacts in a standard format, and uses AI capabilities to do things a traditional UX process cannot — like stress-testing assumptions against synthesized user archetypes, generating competing design hypotheses in parallel, and proactively surfacing anti-patterns. Always trigger this skill before any prototype or flow work begins. If the user jumps straight to screens without personas or goals, invoke this skill to pull the process back upstream.
---
